...
export function CollectionCard(collection: Subcategory) {
  const words = collection.name.split(" ");
  const router = useRouter();
  return (
    <Link
      href={`/${collection.categorySlug}/new-arrivals/${collection.slug}`}
      className={cn(
    ...
    >
      ...
      <div className="relative inset-0 transition-opacity duration-500 lg:min-h-[541px] sm:min-h-[541px] min-h-[339px]">
        {collection.thumbNailType != "video" ? (
          ...
          <div className="w-full lg:h-[541px] sm:h-[541px] h-[339px]">
            <VideoPlayerCustom

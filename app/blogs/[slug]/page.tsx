    ...
    <div className="max-w-[1130px] mx-auto pb-20 mt-[20px] px-4">
      ...
      <div className="flex flex-col lg:flex-row gap-[25px]">
        <div className="w-[100%] lg:w-[70%]">
          ...
          <div className="text-[#ffffff99] text-[16px] font-lighter text-justify tracking-[0.6px] leading-7 mt-6" dangerouslySetInnerHTML={{ __html: blog.description }} />
              ...
              <div key={index} className="single-description mb-8">
                ...
                {item.youtube && (
                  <div className="youtube-container mt-6">
                    <iframe width="100%" height="433" src={convertToEmbedUrl(item.youtube)} title="YouTube video" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen />
                  </div>
                )}
                {item.multipleImages && item.multipleImages.length > 0 && (
                  <div className={`gallery-container grid gap-4 mt-6 ${item.multipleImages.length === 1 ? "grid-cols-1" : item.multipleImages.length === 2 ? "grid-cols-2" : item.multipleImages.length === 3 ? "grid-cols-3 grid-rows-2" : item.multipleImages.length === 4 ? "grid-cols-2 grid-rows-2" : "grid-cols-3 grid-rows-2"}`}>
                ...
                {item.button?.hyperLink && item.button?.title && (
                  <div className="button mt-6">
                    <Link href={item.button.hyperLink} className="bg-white text-black py-2 px-6 rounded-lg inline-block" target="_blank" rel="noopener noreferrer">
                      {item.button.title}
                    </Link>
                  </div>
                )}
        ...
        <div className="w-[100%] lg:w-[30%] sticky self-start top-[90px]">
          <div className="bg-[#1a1a1a] p-5 mb-[25px] rounded-lg pb-6">
            <h3 className="text-lg text-[#ffffff99] mb-4 border-b border-[#ffffff33] pb-2">Explore Other Blogs</h3>
            <ExploreOtherBlogs allblogs={allblogs} currentSlug={params.slug} />
          </div>
          <div className="bg-[#1a1a1a] p-5 rounded-lg pb-6">
            <h3 className="text-lg text-[#ffffff99] mb-4 border-b border-[#ffffff33] pb-2">Tags</h3>
            <div className="flex flex-wrap gap-2">
              {blog.seoDetails.tags?.map((tag, index) => (
                <Link key={index} href="#" className="text-[#ffffff99] hover:text-white hover:bg-black cursor-context-menu bg-[#333] px-2 py-1 rounded-md">
                  {tag}
                </Link>
              ))}
            </div>
